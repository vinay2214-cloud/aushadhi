...
> Framework: Google ADK + Antigravity SDK | Model: Gemini 3.5 Flash
...
**Uses Gemini:** YES — Gemini 3.5 Flash  
...
**Gemini API Config (both prompts) — validated working config, do not deviate:**

Uses the `google-genai` SDK (NOT the deprecated `google-generativeai` package) on
model `gemini-3.5-flash`. This model has "thinking" enabled by default, and thinking
tokens are deducted from the SAME budget as `max_output_tokens` — without capping
`thinking_level`, the model can silently exhaust its token budget reasoning and
return a truncated response (`finish_reason=MAX_TOKENS`) even at generous limits.
`temperature` is intentionally omitted — Gemini 3.5 no longer recommends setting it.
...
response = client.models.generate_content(
    ...
    config=types.GenerateContentConfig(
        ...
        max_output_tokens=8192,            # generous — thinking + JSON output share this budget
        response_mime_type="application/json",
        thinking_config=types.ThinkingConfig(thinking_level="low"),
    ),
)

# Parse defensively — Gemini occasionally appends stray characters after
# valid JSON. Use raw_decode to take only the first complete object.
decoder = json.JSONDecoder()
result, _ = decoder.raw_decode(response.text.strip())
```

**Retry policy:** wrap the call with retry-on-503 (server overload is common and
transient — wait ~8s and retry, up to 4 attempts) using `tenacity` or the pattern
already in `scripts/test_gemini_outbreak.py`.
...
class ForecastOutbreakAgent(BaseAgent):
    ...
    async def get_model(self) -> Any:
        ...
        return self.gemini_client      # Gemini 3.5 Flash
```
